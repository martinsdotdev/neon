...
/** Records repository interactions across a set of in-memory async repositories, so suites can
  * assert cross-repository ordering (e.g. task saved before stock, stock before shortpick) and that
  * conditional loads were skipped. Share one instance across all repositories in a suite.
  */
