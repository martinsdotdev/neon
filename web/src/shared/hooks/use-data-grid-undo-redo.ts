...
import { getIsInPopover } from "@/shared/data-grid/lib"
...
interface HistoryEntry<TData> {
  ...
  count: number
  timestamp: number
  undo: (currentData: Array<TData>) => Array<TData>
  redo: (currentData: Array<TData>) => Array<TData>
}
...
interface StoreState<TData> {
  undoStack: Array<HistoryEntry<TData>>
  redoStack: Array<HistoryEntry<TData>>
...
interface UseDataGridUndoRedoReturn<TData> {
  ...
  onRedo: () => void
  onClear: () => void
  trackCellsUpdate: (updates: Array<UndoRedoCellUpdate>) => void
  trackRowsAdd: (rows: Array<TData>) => void
  trackRowsDelete: (rows: Array<TData>) => void
}
...
function useDataGridUndoRedo<TData>({
  ...
  const trackCellsUpdate = React.useCallback(
    (updates: Array<UndoRedoCellUpdate>) => {
