  ...
  override protected def processEvent(
    ...
    envelope.event match
      case SlotActor.Initialized(slot) =>
        val stmt = session
          .createStatement(SlotByWorkstation.Upsert)
      ...
      case SlotActor.DomainEvent(e) =>
        e match
          case e: SlotEvent.SlotReserved =>
            val stmt = session
              .createStatement(SlotByWorkstation.UpdateOrderIdAndState)
          ...
          case e: SlotEvent.SlotCompleted =>
            val stmt = session
              .createStatement(SlotByWorkstation.UpdateState)
              .bind(0, "Completed")
          ...
          case e: SlotEvent.SlotReleased =>
            val stmt = session
              .createStatement(SlotByWorkstation.ClearOrderIdAndUpdateState)
              .bind(0, "Available")
              .bind(1, e.slotId.value)
            session.updateOne(stmt).map(_ => Done)
