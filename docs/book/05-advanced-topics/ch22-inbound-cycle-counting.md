...
`Received` and `Closed` both carry `receivedQuantity` and
...
The stock position update is optional. Some warehouses update stock
...
Blind counting is the preferred method for high-value SKUs and any
...
The snapshot captures a point-in-time view of stock. If a pick
...
The tolerance threshold and maximum recount depth are
