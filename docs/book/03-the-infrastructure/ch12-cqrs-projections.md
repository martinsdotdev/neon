...
The `settings = None` parameter means we accept Pekko's default
...
Every handler class lives in the `app` module under
...
The `LazyLogging` trait from scala-logging creates one logger per
