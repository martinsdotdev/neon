...
class UomHierarchySuite extends AnyFunSpec:
  describe("UomHierarchy"):
    it("maps packaging levels to eaches-per-unit"):
    ...
    it("supports direct access and membership check"):
    ...
    it("rejects Each — it is the implicit base unit"):
      assertThrows[IllegalArgumentException]:
        UomHierarchy(PackagingLevel.Each -> 1)

    it("rejects zero or negative eaches-per-unit"):
      assertThrows[IllegalArgumentException]:
