...
> **Note:** If `isFullyReceived` were false, `complete` would throw via
...
> **Note:** Each created task now carries a `stockPositionId` linking it
...
> **Note:** The adjustment flow is deliberately two-step: first
...
> **Note:** The `assign` method on `Idle` takes a `UUID` assignment
