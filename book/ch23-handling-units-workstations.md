...
> **Note:** Both streams share the same `HandlingUnitId` type and the same
...
> **Note:** Transport orders are deliberately decoupled from the task that
...
> **Note:** Disabling an `Active` workstation does not automatically cancel
...
> **Note:** Slot completion does not automatically advance the ship handling
